...
class Board_m extends CI_Model {
    ...
    function insert_board($post) //글 작성
	{
		...
		if(MENU_ID == '10')
		{
			...
			$this->email->from('a@a.com', '조이헌팅');
			$this->email->to('a@a.com');

			$this->email->subject("[조이헌팅] ".$post['subject']);
			$e_content =
		...
		if(MENU_ID != '10')
		{
			...
			$this->email->subject("[조이헌팅 ".MENU_BOARD_NAME."] ".$post['subject']);
	...
	{
		$where = "";
		$this->db->select($table.'.user_id,'.$table.'.user_name, '.$table.'.subject, '.$table.'.no,'.$table.'.hit, '.$table.'.voted_count,  '.$table.'.contents, '.$table.'.reg_date, '.$table.'.reply_count, files.file_name');
