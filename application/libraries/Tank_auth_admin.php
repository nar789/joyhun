...
define('ADMIN_PHPASS_HASH_STRENGTH', 8);
define('ADMIN_PHPASS_HASH_PORTABLE', FALSE);

define('ADMIN_STATUS_ACTIVATED', '1');
define('ADMIN_STATUS_NOT_ACTIVATED', '0');
...
class Tank_auth_admin
{
	...
	function __construct()
	{
		...
		$this->ci->load->model('tank_auth/admin_users');
	...
	function login($login, $password, $remember, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->admin_users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$this->ci->session->set_userdata(array(
								...
								'admin_status'	=> ($user->activated == 1) ? ADMIN_STATUS_ACTIVATED : ADMIN_STATUS_NOT_ACTIVATED,
						));
						...
						if ($user->activated == 0) {							// fail - not activated
							...
							$this->ci->admin_users->update_login_info(
									$user->id,
...
//	{
//		return $this->ci->session->userdata('admin_status') === ($activated ? ADMIN_STATUS_ACTIVATED : ADMIN_STATUS_NOT_ACTIVATED);
//	}

	function is_logged_in($activated = TRUE)
	{
		if($this->ci->session->userdata('auth_code') > 0){
			return true;
		}else{
			return false;
		}	
	...
	{
		if ((strlen($userid) > 0) AND !$this->ci->admin_users->is_userid_available($userid)) {
			$this->error = array('userid' => 'auth_userid_in_use');

		}
		elseif (!$this->ci->admin_users->is_nickname_available($nickname)) {
			$this->error = array('nickname' => 'auth_nickname_in_use');
		}
		/*else if ((strlen($username) > 0) AND !$this->ci->admin_users->is_username_available($username)) {
			$this->error = array('username' => 'auth_username_in_use');

		} */
		elseif (!$this->ci->admin_users->is_email_available($email)) {
			...
			if (!is_null($res = $this->ci->admin_users->create_user($data, !$email_activation))) {
	...
	{
		return ((strlen($username) > 0) AND $this->ci->admin_users->is_username_available($username));
	}
	...
	{
		return ((strlen($email) > 0) AND $this->ci->admin_users->is_email_available($email));
	}
	...
	function change_email($email)
	{
		...
		if (!is_null($user = $this->ci->admin_users->get_user_by_id($user_id, FALSE))) {
			...
			if ($user->email == $email) {		// leave activation key as is
				...
				$data['new_email_key'] = md5(rand().microtime());
				$this->ci->admin_users->set_new_email($user_id, $email, $data['new_email_key'], FALSE);
				return $data;
	...
	{
		$this->ci->admin_users->purge_na($this->ci->config->item('email_activation_expire', 'tank_auth'));

		if ((strlen($user_id) > 0) AND (strlen($new_email_key) > 0)) {
			return $this->ci->admin_users->activate_user($user_id, $new_email_key);
		}
		return FALSE;
	}
	...
	{
		if (strlen($login) > 0) {
			if (!is_null($user = $this->ci->admin_users->get_user_by_login($login, TRUE))) {
				...
				$this->ci->admin_users->set_password_key($user->id, $data['new_pass_key']);
				return $data;
	...
	function can_reset_password($user_id, $new_pass_key)
	{
		if ((strlen($user_id) > 0) AND (strlen($new_pass_key) > 0)) {
			return $this->ci->admin_users->can_reset_password(
				$user_id,
				$new_pass_key,
	...
	function reset_password($user_id, $new_pass_key, $new_password)
	{
		if ((strlen($user_id) > 0) AND (strlen($new_pass_key) > 0) AND (strlen($new_password) > 0)) {
			...
			if (!is_null($user = $this->ci->admin_users->get_user_by_id($user_id, TRUE))) {

				// Hash password using phpass
				$hasher = new PasswordHash(ADMIN_PHPASS_HASH_STRENGTH, ADMIN_PHPASS_HASH_PORTABLE);
				$hashed_password = $hasher->HashPassword($new_password);

				if ($this->ci->admin_users->reset_password(
						$user_id,
						$hashed_password,
						$new_pass_key,
					...
					$this->ci->load->model('tank_auth/admin_user_autologin');
					$this->ci->admin_user_autologin->clear($user->id);
	...
	function change_password($old_pass, $new_pass)
	{
		...
		if (!is_null($user = $this->ci->admin_users->get_user_by_id($user_id, TRUE))) {
			...
			if ($hasher->CheckPassword($old_pass, $user->password)) {			// success
				...
				$this->ci->admin_users->change_password($user_id, $hashed_password);
				return TRUE;
	...
	function set_new_email($new_email, $password)
	{
		...
		if (!is_null($user = $this->ci->admin_users->get_user_by_id($user_id, TRUE))) {
			...
			if ($hasher->CheckPassword($password, $user->password)) {			// success
				...
				} elseif ($this->ci->admin_users->is_email_available($new_email)) {
					$data['new_email_key'] = md5(rand().microtime());
					$this->ci->admin_users->set_new_email($user_id, $new_email, $data['new_email_key'], TRUE);
					return $data;
	...
	{
		if ((strlen($user_id) > 0) AND (strlen($new_email_key) > 0)) {
			return $this->ci->admin_users->activate_new_email(
					$user_id,
					$new_email_key);
		}
		return FALSE;
	...
	function delete_user($password)
	{
		...
		if (!is_null($user = $this->ci->admin_users->get_user_by_id($user_id, TRUE))) {
			...
			if ($hasher->CheckPassword($password, $user->password)) {			// success
				...
				$this->ci->admin_users->delete_user($user_id);
				$this->logout();
				return TRUE;
	...
	private function create_autologin($user_id)
	{
		...
		$this->ci->load->model('tank_auth/admin_user_autologin');
		$this->ci->admin_user_autologin->purge($user_id);

		if ($this->ci->admin_user_autologin->set($user_id, md5($key))) {
	...
	private function delete_autologin()
	{
		...
		if ($cookie = get_cookie($this->ci->config->item('autologin_cookie_name', 'tank_auth'), TRUE)) {
			...
			$this->ci->load->model('tank_auth/afmin_user_autologin');
			$this->ci->admin_user_autologin->delete($data['user_id'], md5($data['key']));
		...
		if (!$this->is_logged_in() AND !$this->is_logged_in(FALSE)) {			// not logged in (as any user)
			...
			if ($cookie = get_cookie($this->ci->config->item('autologin_cookie_name', 'tank_auth'), TRUE)) {
				...
				if (isset($data['key']) AND isset($data['user_id'])) {
					...
					$this->ci->load->model('tank_auth/admin_user_autologin');
					if (!is_null($user = $this->ci->admin_user_autologin->get($data['user_id'], md5($data['key'])))) {

						// Login user
						$this->ci->session->set_userdata(array(
								'user_id'	=> $user->id,
								'username'	=> $user->username,
								'admin_status'	=> ADMIN_STATUS_ACTIVATED,
						));
	...
	function is_max_login_attempts_exceeded($login)
	{
		if ($this->ci->config->item('login_count_attempts', 'tank_auth')) {
			$this->ci->load->model('tank_auth/admin_login_attempts');
			return $this->ci->admin_login_attempts->get_attempts_num($this->ci->input->ip_address(), $login)
	...
	private function increase_login_attempt($login)
	{
		if ($this->ci->config->item('login_count_attempts', 'tank_auth')) {
			if (!$this->is_max_login_attempts_exceeded($login)) {
				$this->ci->load->model('tank_auth/admin_login_attempts');
				$this->ci->admin_login_attempts->increase_attempt($this->ci->input->ip_address(), $login);
			}
		}
	}
	...
	private function clear_login_attempts($login)
	{
		if ($this->ci->config->item('login_count_attempts', 'tank_auth')) {
			$this->ci->load->model('tank_auth/admin_login_attempts');
			$this->ci->admin_login_attempts->clear_attempts(
