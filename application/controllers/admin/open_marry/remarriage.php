...
class Remarriage extends MY_Controller {
	...
	{
		$this->remarriage_list();
	}

	function remarriage_list()  //재혼신청 리스트
	{
		...
		$result = $this->my_m->get_list($start, $rp, @$search, 'T_CoupleMarry_ReMarryMan', 'm_idx', 'm_userid', 'desc');
		...
		$this->load->view('admin/open_marry/remarriage_list_v', $data);
		$this->load->view('admin/admin_bottom_v');

	}

	//재혼신청 삭제
	function remarriage_list_del(){
		...
		$rtn = $this->my_m->del('T_CoupleMarry_ReMarryMan',$arr_data);
