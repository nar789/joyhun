...
class Open_marry extends MY_Controller {
	...
	{
		$this->open_guhon_list();
	}

	function open_guhon_list()  //결혼신청 리스트
	{
		...
		$this->load->view('admin/open_marry/open_marry_list_v', $data);
		$this->load->view('admin/admin_bottom_v');

	}

	//공개구혼 선택 삭제
	function open_guhon_list_del(){
	
		$b_num = urldecode($this->security->xss_clean(url_explode($this->seg_exp, 'b_num')));
		$b_userid = urldecode($this->security->xss_clean(url_explode($this->seg_exp, 'b_userid')));
		...
		$rtn = $this->my_m->del('T_CoupleMarry_OpenguhonMan',$arr_data);
