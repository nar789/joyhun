...
class Chat extends MY_Controller {
	...
	function chat_request(){
		...
		//채팅신청을 한회원에게 계속된 채팅신청할경우 기존 신청내역 삭제처리하기(계속 채팅신청 허용)
	...
	function chat_accept(){
		...
		//포인트 차감 체크를 위해 채팅 수락하는 회원의 성별 체크하여 포인트 차감 기본 아이디 셋팅
		if($this->session->userdata['m_sex'] == "M"){
		...
		//회원 보유 포인트 체크하기(point_helper) $m_userid = 포인트차감 대상 아이디, $chat_point = 차감포인트
		$mpc = member_point_chk($m_userid, $chat_point);

		if($mpc == "success"){
			//남성의 포인트가 있을경우

			$send_id = $user_id;									//보낸회원 아이디 셋팅
			$recv_id = $this->session->userdata['m_userid'];		//받는회원 아이디 셋팅
			...
			$req_idx = $req_data['idx'];		//채팅방 번호 셋팅

			//해당 채팅방 번호에 수락 내역이 있는지 체크하기
			...
			if(!empty($accept_data)){
				...
				if($accept_data['is_delete_gubn'] == "D"){
					...
					//삭제처리된 채팅방은 아니지만 이미 수락된 채팅방의 경우
					echo "accept";
					exit;
				}
			}
	...
	function chat_deny(){
		...
		$user_id	= rawurldecode($this->security->xss_clean(@url_explode($this->seg_exp, 'user_id')));		//채팅 신청자 아이디
		$deny_msg	= rawurldecode($this->security->xss_clean(@url_explode($this->seg_exp, 'deny_msg')));		//채팅 거절 메세지
		
		if(empty($user_id)){ exit; }		//상대방 아이디가 없을경우 중지
		$my_id		= $this->session->userdata['m_userid'];
		
		//채팅방 번호 가져오기
		$req_data = $this->my_m->row_array('chat_request', array('send_id' => $user_id, 'recv_id' => $my_id, 'status' => '', 'is_delete' => null), 'idx', 'desc', '1');
		if(empty($req_data)){ exit; }		//채팅 신청 내역이 없을 경우 중지

		$req_idx = $req_data['idx'];		//채팅방 번호
		//채팅방 거절 처리
		$this->my_m->update('chat_request', array('idx' => $req_idx), array('status' => '거절', 'deny_id' => $my_id));
		
		//신규 알림 메세지 줄이기
		$this->chat_m->mod_alrim_new(null, $user_id);

		//채팅 거절 메세지 남기기

		//채팅 거절 메시지 남기기(상대방)

		//거절 사유가 있을경우 거절메세지 추가

		//채팅 거절시 신청자의 채팅 모두 읽음 처리
	...
	function chat_request_mobile(){
		...
		//채팅신청을 한회원에게 계속된 채팅신청시 기존 채팅방 삭제 처리(계속된 채팅신청 허용)
