...
class Event_winter extends MY_Controller {
	...
	function index(){
		//if($_SERVER['REMOTE_ADDR'] <> "14.47.36.51"){ alert_goto('준비중입니다.', '/'); }
		$this->winter();
	}

	function winter(){	// 이벤트 뷰페이지		
		...
		//이벤트 토크 리스트 가져오기
		$result = $this->my_m->get_list($start, $rp, array('t_gubn' => '3'), 'T_JoyTalk', 't_idx', 't_id', 'desc', '*');
		...
		if(IS_MOBILE == true){
			...
			$this->load->view('service_center/event_winter_v',$data);
	...
	function talk_list_more(){
		...
		$m_result = $this->my_m->get_list($start, $rp, array('t_gubn' => '3'), 'T_JoyTalk', 't_idx', 't_id', 'desc');
		$mlist = $m_result[0];
