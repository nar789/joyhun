...
class Chat extends MY_Controller {
	...
	//일대일 채팅신청 처리
		...
		$this->chat_lib->set_recv_id($user_id);
		$rtn =  $this->chat_lib->chat_request_submit($contents,@$v_gubn);
	...
	function chatting_submit(){
		...
		//채팅 보내기 (라이브러리 사용)
		$this->chat_lib->set_recv_id($user_id);
		$rtn = $this->chat_lib->chatting_submit($contents);
	...
	function chat_deny(){
		...
		//채팅 보내기 (라이브러리 사용)
		$this->chat_lib->set_send_id($this->session->userdata['m_userid']);
		$this->chat_lib->set_recv_id($user_id);
		...
		if(!empty($deny_msg)){
			...
			$this->chat_lib->chatting_save($deny_msg);
