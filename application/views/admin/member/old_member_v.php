...
$(document).ready(function(){
	$("#search_btn").click(function(){
		...
		if($("#q").val() == '' && $("#q2").val() == ''){
			...
			var act = '/admin/main/old_member_list';
			if($("#q").val() && sfl_val){
...
function old_member_new(user_id){
	...
	if(confirm("해당 회원을 휴면상태에서 해제하시겠습니까?") == true){

		$.ajax({

			type : "post",
			url : "/admin/main/call_old_member_new",
			data : {
...
<?php
$sfl_arr = array('m_userid'=>'아이디',  'm_name'=>'이름', 'm_nick'=>'닉네임', 'm_jumin1'=>'주민번호', 'm_hp'=>'핸드폰번호', 'm_mail'=>'메일주소');
...
<?php
$sfl_arr = array('m_name'=>'이름', 'm_userid'=>'아이디', 'm_nick'=>'닉네임',  'm_jumin1'=>'주민번호', 'm_hp'=>'핸드폰번호', 'm_mail'=>'메일주소');
							...
							<div class="table-responsive">
								<table class="table table-bordered table-vertical-middle nomargin">
									...
									<tbody>
										...
										?>
										<tr>
											<td><a href="<?=ADMIN_DIR?>main/member_view/userid/<?=$data['m_userid']?>/gubn/old"><?php echo $data['m_userid']?></a></td>
											...
											<td>
												<input type="button" id="btn_old" name="btn_old" value="해제" class="btn btn-danger" style="width:80px; height:30px; font-weight:bold;" onclick="javascript:old_member_new('<?=$data['m_userid']?>')">
											</td>
										</tr>
										<?
											}
