	...
	$(document).ready(function(){
		$("#search_btn").click(function(){
			...
			if($("#q").val() == ''){
				...
				var act = '/admin/friend/anne/anne_list';
				if($("#q").val() && sfl_val){
	...
	function del_list(m_idx, m_userid){
		...
		if(confirm("정말 삭제하시겠습니까?")){
			$.ajax({
				...
				type : "post",
				url : "/admin/friend/anne/del_anne/m_idx/"+m_idx,
				cache : false,
				async : false,
		...
		if(chk_cnt == 0){
			...
			$("input[name='choice_chk']:checked").each(function() {
				...
				$.ajax({
					...
					type : "post",
					url : "/admin/friend/anne/del_anne/m_idx/"+de_idx,
					cache : false,
					async : false,
					success : function(result){},
					error : function(result){
...
<section id="middle">
	...
	<div id="content" class="padding-20">
		...
		<div id="panel-2" class="panel panel-default">
			<div class="panel-heading">
				<span class="title elipsis">
					<strong>앤만들기 리스트</strong> <!-- panel title -->
				</span>
					...
					<table class="table table-bordered table-vertical-middle nomargin">
						...
						<tbody>
							...
							<tr>
								...
								<td><?=$data['m_dbcontent']?></td>
								<td class="text-center"><nobr><?=$data['m_date']?></nobr></td>
