...
function alrim_close(obj, num) {
  $(obj + num).fadeOut("slow", function () {
    open_status(obj);
  });
}

function alrim_close2(obj, num) {
  $(obj + num).hide(function () {
    open_status(obj);
  });
}
...
function open_alrim(obj) {
  ...
  z_index = z_index + 1;
  $(obj + num).fadeIn("slow");
  $(obj + num).animate({ bottom: 0 }, 800, function () {
    setTimeout(function () {
      alrim_close(obj, num);
    }, 5000);
  });
    ...
    if (data.length > 3) {
      ...
      if (
        ...
        if (datas["chat_new_cnt"] > 0) {
          open_status("#chat_alrim");
        }
        if (datas["msg_new_cnt"] > 0) {
          open_status("#msg_alrim");
        }
        if (datas["joy_new_cnt"] > 0) {
          open_status("#joy_alrim");
        }
      }
    }
  });
...
function set_alrim(datas) {
  if (datas["chat_pop_idx"]) {
    ...
    $("#alrim_wrap")[0].innerHTML =
      ...
      '<b class="color_333 font-size_12 font_900"><span class="border_bottom_1_333">' +
      decodeURIComponent(datas["chat_new_nick"]) +
      "</span>님</b>이 채팅신청을 하였습니다." +
      "</p>" +
      '<div class="width_85 block ver_top margin_top_9">' +
      decodeURIComponent(datas["chat_new_pic"]) +
      "</div>" +
      '<div class="block margin_top_9">' +
      '<textarea class="chat_view" readonly>' +
      chat_new_text +
      "</textarea>" +
      "</div>" +
      '<div class="margin_top_6 text-center alrim_buttons">' +
      '<input type="button" class="text_btn_de4949 alarm_chatting_btn" value="수락" onclick=chat_accept_flg("' +
      datas["chat_new_userid"] +
      '");>' +
      '<input type="button" class="text_btn_ea3e3e alarm_chatting_btn" value="거절" onclick=chat_deny("' +
      datas["chat_new_userid"] +
      '");>' +
      "</div>" +
      "</div>" +
    ...
    if (localStorage.getItem("chat_" + datas["msg_new_userid"]) != "true") {
      ...
      $("#alrim_wrap")[0].innerHTML =
        ...
        '<b class="color_333 font-size_12 font_900"><span class="border_bottom_1_333 pointer" onclick=openChatWindow("' +
        datas["msg_new_userid"] +
        '");>' +
        cutStr(msg_new_text, 80) +
        "</span></b>" +
        '<p class="color_666 font-size_12 margin_top_8 pointer" onclick=openChatWindow("' +
        datas["msg_new_userid"] +
        '");>' +
        msg_new_nick +
        "님으로부터 메시지가 <br> 도착했습니다." +
        "</p>" +
        "</div>" +
        "</div>" +
        "</div>" +
    ...
    if (datas["joy_new_pic"]) {
      ...
      d_width = "width_248 height_73";
    }
    chat_buff +=
      '<div class="alarm_text ' +
      d_width +
      '">' +
      '<p class="color_666 font-size_12 margin_top_8 pointer" onclick=go_msg_alrim("' +
      decodeURIComponent(datas["joy_pop_url"]) +
      '");>' +
      joy_new_text +
      "</p>";
    if (datas["event_btn_text"]) {
      chat_buff +=
        '<p><input type="button" class="text_btn_dcdcdc alarm_jjim_btn" value="' +
        decodeURIComponent(datas["event_btn_text"]) +
        '" onclick=go_msg_alrim("' +
        decodeURIComponent(datas["joy_pop_url"]) +
        '");></p>';
    }
    chat_buff += "</div>" + "</div>" + "</div>" + "</div>";
...
function set_status(datas) {
  if (datas["chat_new_cnt"] > 0) {
    if (datas["chat_new_text"] == undefined) {
      datas["chat_new_text"] = "";
    }
    chat_new_text = decodeURIComponent(datas["chat_new_text"]);
    chat_new_text = chat_new_text.replace(/'/g, "\\'");

    if (datas["chat_new_cnt"] > 99) {
      datas["chat_new_cnt"] = 99;
    }
    ...
    $("#alrim_chat_status_wrap")[0].innerHTML =
      '<div class="alarm_area2" id="chat_alrim_status" ' +
      add_style +
      ">" +
      '<div class="alarm_small_list bg_e15148" onclick=open_alrim_list("#chat_alrim");>' +
      '<span class="alarm_small_title">채팅신청 수락</span>' +
      '<div class="alarm_cnt" id="chat_status_cnt">' +
      datas["chat_new_cnt"] +
      "</div>" +
      '<span class="alarm_small_text"  id="chat_status_text">' +
      cutStr(chat_new_text, 30) +
      "</span>" +
      "</div>" +
      "</div>";
  ...
  if (datas["msg_new_cnt"] > 0) {
    if (datas["msg_new_text"] == undefined) {
      datas["msg_new_text"] = "";
    }
    msg_new_text = decodeURIComponent(datas["msg_new_text"]);
    msg_new_text = msg_new_text.replace(/'/g, "\\'");

    if (datas["msg_new_cnt"] > 99) {
      datas["msg_new_cnt"] = 99;
    }
    ...
    $("#alrim_msg_status_wrap")[0].innerHTML =
      '<div class="alarm_area2" id="msg_alrim_status" ' +
      add_style +
      ">" +
      '<div class="alarm_small_list bg_8ab933" onclick=open_alrim_list("#msg_alrim");>' +
      '<span class="alarm_small_title">메시지도착 알림</span>' +
      '<div class="alarm_cnt" id="msg_status_cnt">' +
      datas["msg_new_cnt"] +
      "</div>" +
      '<span class="alarm_small_text"  id="msg_status_text">' +
      cutStr(msg_new_text, 30) +
      "</span>" +
      "</div>" +
      "</div>";
  ...
  if (datas["joy_new_cnt"] > 0) {
    if (datas["joy_new_text"] == undefined) {
      datas["joy_new_text"] = "";
    }
    joy_new_text = decodeURIComponent(datas["joy_new_text"]);
    joy_new_text = joy_new_text.replace(/'/g, "\\'");

    if (datas["joy_new_cnt"] > 99) {
      datas["joy_new_cnt"] = 99;
    }
    ...
    $("#alrim_joy_status_wrap")[0].innerHTML =
      '<div class="alarm_area2" id="joy_alrim_status" ' +
      add_style +
      ">" +
      '<div class="alarm_small_list bg_5bc0de" onclick=open_alrim_list("#joy_alrim");>' +
      '<span class="alarm_small_title">조이헌팅 알림</span>' +
      '<div class="alarm_cnt" id="joy_status_cnt">' +
      datas["joy_new_cnt"] +
      "</div>" +
      '<span class="alarm_small_text"  id="joy_status_text">' +
      cutStr(joy_new_text, 30) +
      "</span>" +
      "</div>" +
      "</div>";
...
function open_chat_win(user_id, status, req_idx) {
  if (status == "다시채팅" || status == "수락대기" || status == "상대나감") {
    ...
    $.get(
      "/profile/my_chat/chat_room_chk/user_id/" +
        user_id +
        "/req_idx/" +
        req_idx +
        "/" +
...
function openChatWindow(user_id) {
  win = window.open(
    "/chat/chatting/" + user_id,
    "chat_" + user_id,
    "fullscreen=0,toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=0,resizable=yes,width=400,height=700;"
  );
}
  ...
  $.get("/chat/hook_get_alrim_chat_list/" + Math.random(), function (data) {
    if (data.length > 3) {
      ...
      $.each(data.split("¶"), function (index, x1) {
        ...
        chat_buff +=
          '<div class="alarm_content_box" id="chat_list_box_' +
          datas["idx"] +
          '">' +
          "<a href=\"javascript:chat_list_item_close('" +
          datas["idx"] +
          '\');"><img src="/images/meet_exit_btn.gif" class="alarm_exit_btn"></a>' +
          '<p class="color_666 font-size_12">' +
          '<b class="color_333 font-size_12 font_900"><span class="border_bottom_1_333">' +
          send_user_nick +
          "</span>님</b>이 채팅신청을 하였습니다." +
          ...
          '<div class="margin_left_65 alrim_buttons"  id="chat_btn_' +
          datas["idx"] +
          '" ' +
          div_style1 +
          ">" +
          ...
          ");>" +
          "</div>" +
          '<div class="margin_left_100"  id="chat_accept_' +
          datas["idx"] +
          '" ' +
          div_style2 +
          ">" +
          '<input type="button" class="text_btn_de4949 alarm_chatting_btn" value="다시채팅" onclick=openChatWindow("' +
          datas["send_id"] +
          '");>' +
          "</div>" +
          '<div class="margin_left_85 chat_deny_text" id="chat_deny_' +
          datas["idx"] +
          '" ' +
          div_style3 +
          ">" +
  ...
  $.get("/chat/hook_get_alrim_msg_list/" + Math.random(), function (data) {
    if (data.length > 3) {
      ...
      $.each(data.split("¶"), function (index, x1) {
        ...
        chat_buff +=
          '<div class="alarm_content_area border_left_right_none" id="msg_list_box_' +
          datas["idx"] +
          '">' +
          '<div class="alarm_content_box">' +
          "<a href=\"javascript:msg_list_item_close('" +
          datas["idx"] +
          '\');"><img src="/images/meet_exit_btn.gif" class="alarm_exit_btn"></a>' +
          '<div class="width_85 height_73 block ver_top">' +
          decodeURIComponent(datas["thumb"]) +
  ...
  $.get("/chat/hook_get_alrim_joy_list/" + Math.random(), function (data) {
    if (data.length > 3) {
      ...
      $.each(data.split("¶"), function (index, x1) {
        ...
        chat_buff +=
          '<div class="alarm_content_area border_left_right_none" id="joy_list_box_' +
          datas["idx"] +
          '">' +
          '<div class="alarm_content_box">' +
          "<a href=\"javascript:joy_list_item_close('" +
          datas["idx"] +
          '\');"><img src="/images/meet_exit_btn.gif" class="alarm_exit_btn"></a>';
        ...
        if (datas["event_btn_text"]) {
          chat_buff +=
            '<p><input type="button" class="text_btn_dcdcdc alarm_jjim_btn" value="' +
            datas["event_btn_text"] +
            '" onclick=go_msg_alrim("' +
            datas["event_url"] +
            '");></p>';
        }
        chat_buff += "</div>" + "</div>" + "</div>";
      });

      document.getElementById(
        "alrim_joy_list_wrap_inlist"
      ).innerHTML = chat_buff;
      chat_buff = "";
    }
  });
...
function call_message_data() {
  ...
  $.get(
    "/profile/message/call_message_alrim/" + Math.random(),
    function (data) {
      if (data) {
        result = data;
