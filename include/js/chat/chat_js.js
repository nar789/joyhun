...
function chat_fist_load(){
	...
	user_id = $("#user_id").val();
	$.get('/chat/hook_get_chat_first_load/'+user_id+'/'+Math.random(), function(data){
...
function add_chat_send(datas, reg_date, buffer){
			...
			if(last_date != today){
				//날짜넣기
				chat_add_table += "<div class='day_area'><span class='day_text'>"+now.getFullYear()+"년 "+(now.getMonth()+1)+"월 "+now.getDate()+"일 "+week[now.getDay()]+"</span></div>";
				last_date = today;
			...
			if(datas['mode'] == "chat" || datas['mode'] == "chat_req"){ //일반채팅
			last_chat = "send";
				...
				chat_add_table += "<p class='read_cnt' id='chat_read_cnt_"+datas['idx']+"'> "+datas['read_cnt']+" </p> " + reg_date;
				chat_add_table += "</td>";
				...
				last_chat = "send";
				chat_add_table += call_chat_list_view('send', datas['contents'], datas['recv_id']);
			}else if(datas['mode'] == "gift_req"){
				last_chat = "send";
				chat_add_table += call_chat_list_view_req('send', datas['contents'], datas['recv_id']);
			}
...
function add_chat_recv(datas, reg_date, buffer){
			...
			last_chat = "recv";
				...
				last_chat = "recv";
				chat_add_table += call_chat_list_view('recv', datas['contents'], datas['send_id']);

			}else if(datas['mode'] == "gift_req"){
				last_chat = "recv";
				chat_add_table += call_chat_list_view_req('recv', datas['contents'], datas['send_id']);
			}
...
function chat_ajax_call(){
	user_id = $("#user_id").val();
	$.get('/chat/hook_get_chat/'+user_id+'/'+Math.random(), function(data){
...
function chat_exit(user_id){
	...
	if(confirm("채팅방을 나가시면 더이상 채팅이 불가능합니다.\n채팅방을 나가시겠습니까?") == true){
		
		$.get('/chat/chat_exit/user_id/'+encodeURIComponent(user_id)+'/'+Math.random(), function(result){
			if(result == "1"){
...
function chat_request(user_id){
	...
	$.get('/chat/chat_request_mobile/user_id/'+user_id+'/'+Math.random(), function(data){	//작은창을 띄워야하기위해 강제로 chat_request_mobile 호출
